RPGUI = (function() {
  ...
  RPGUI.create = function(element, rpgui_type) {
    ...
    // not a valid type? exception.
      ...
      throw `Not a valid rpgui type! options: ${Object.keys(
        RPGUI.__create_funcs
      )}`;
    }
  };
  ...
  function mouseDown(e) {
    ...
    if (!RPGUI.has_class(target, _draggable_class)) {
      return;
    }
  ...
  RPGUI.__set_funcs["progress"] = function(elem, value) {
    ...
    // get the two edges
    var edge_left = RPGUI.get_child_with_class(
      elem,
      "rpgui-progress-left-edge"
    );
    var edge_right = RPGUI.get_child_with_class(
      elem,
  ...
  function create_progress(elem) {
    ...
    // set color
    if (RPGUI.has_class(elem, "blue")) {
      progress.className += " blue";
    }
    if (RPGUI.has_class(elem, "red")) {
      progress.className += " red";
    }
    if (RPGUI.has_class(elem, "green")) {
      progress.className += " green";
    }

    // set starting default value
    var starting_val =
      elem.dataset.value !== undefined ? parseFloat(elem.dataset.value) : 1;
  ...
  function create_dropdown(elem) {
    ...
    if (elem.id) {
      select_header.id = `${elem.id}-rpgui-dropdown-head`;
    }
    RPGUI.add_class(
      select_header,
      "rpgui-dropdown-imp rpgui-dropdown-imp-header"
    );
    ...
    if (elem.id) {
      list.id = `${elem.id}-rpgui-dropdown`;
    }
    ...
    (function(elem, select_header, list) {
      ...
      _on_change = function() {
        ...
        if (elem.selectedIndex != -1) {
          select_header.innerHTML =
            arrow_down_prefix + elem.options[elem.selectedIndex].text;
        } else {
  ...
  function create_slider(elem) {
    ...
    if (elem.id) {
      slider_container.id = `${elem.id}-rpgui-slider`;
    }
    ...
    (function(
      ...
      function set_value(new_val) {
        if (!elem.disabled && elem.value != new_val) {
      ...
      function _onchange() {
        ...
        thumb.style.left = `${Math.floor(edges_width * 0.25) +
          relative_val * step}px`;
      }
